...
*/
    ...
    if (!gs.layers.game || !gs.layers.game.drawImage) {return;}
/* * * * * * * * * * * * */
...
/* * * * * * * * * * * * */
    ...
    cvDraw.bg.onload = function(){cvDraw.dBg();}
}
        ...
        for(var y = 0; y < gs.iItems.y; y++){
            for(var x = 0; x < gs.iItems.x; x++){
                gs.layers.game.clearRect(
                    gs.size[gs.actualSize].cll.w * x,
                    gs.size[gs.actualSize].cll.h * y,
                    gs.size[gs.actualSize].cll.w,
                    gs.size[gs.actualSize].cll.h
                );
                ...
                gs.items[y][x] = {
                    ...
                    "timer":{
                        ...
                        "func":function(x,y,back){
                            ...
                            //log('\nscaleX: ' + this._scale + '\ndef: ' + ((1 - this._scale) * gs.size[gs.actualSize].cll.w) + '\nimg.src' + img.src);
                            gs.layers.game.clearRect(
                                gs.size[gs.actualSize].cll.w * x,
                                gs.size[gs.actualSize].cll.h * y,
                                gs.size[gs.actualSize].cll.w,
                            ...
                            gs.layers.game.drawImage(
                                img,
                                gs.size[gs.actualSize].cll.w * x + (1 - this._scale) * gs.size[gs.actualSize].cll.w / 2,
                                gs.size[gs.actualSize].cll.h * y,
                                gs.size[gs.actualSize].cll.w * this._scale,
                                gs.size[gs.actualSize].cll.h
                            );
                        }
                    }
