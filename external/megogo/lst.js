...
catLst.onUp = function(){
	...
	timeoutID = setTimeout(function(){
				if(catLst.pos<6)
						if(currLst == catLst) {
							fl_timeoutID = 1;catLst.onEnter(); //currLst = catLst; currLst.onChange();
						}
			},
	3000);
	}
...
subCatLst.overflow = function(){
  ...
  /*  if(this.length==0&&this.pos==0&&this.prevPos==0&&this.page==0&&catLst.pos == 5){
    ...
    var tmp = 0;
        ...
        if(catLst.pos == 5){  if(this.length == cont_page_max || this.direct == 'prev')
...
function pageSeparating(elem, page){
	 ...
	 lowLimit = (elem.clientHeight-70)*page;     /*$('footer').style.height*/
...
log(back_location)
	...
	if (this.pos == 0)
		window.location= back_location;       // "http://freetv.infomir.com.ua/inet-services/public_html/";
 	else {currLst = prevLst;
 	 $('cats_page').style.display = 'block';
