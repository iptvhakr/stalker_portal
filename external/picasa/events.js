...
function getkeydown ( e ) {
	...
	_debug('power? ec:' + ec + ', event.altKey:' + event.altKey);
	if ( event.altKey && ec === 117 ) {standBy.toggleMode()}   // POWER key
	...
	if ( ec == 32 && e.ctrlKey && !e.altKey ) { ec = ew = 13; }
				...
				case 13: //OK
					if ( curAction == 0 ) {
						...
						if ( curAction == 1 ) {
							...
							drowFooter(4);
							return;
						} else if ( startEdit == 2 && curAction == 2 ) {
							alert('in_progress...');
						} else {
							stb.ShowVirtualKeyboard();
							startEdit++;
			...
			switch ( ec ) {
				...
				case 37: //LEFT
					if ( pictX == 0 ) {
						if ( curPictPage > 0 ) {
							curPictPage--;
							try { document.getElementById('subscript_' + pictY + '' + pictX).className = ' t16w'; } catch ( e ) { }
							drowPicture(curPictPage * maxPictureColum * maxPictureLines);
							if ( pictY ) {
			...
			switch ( ec ) {
				...
				case 112:
					if ( timer ) {
						...
						setTimeout(function () { document.getElementById('paused').innerHTML = ''; }, 2000);
					} else {
						timer = setInterval(function () {
							if ( !slideshowType ) {
								pictureChange('right');
				...
				case 116:
					switch ( pictureAngle ) {
						case 0:
							...
							_debug('realheight ' + realheight + ', ' + 'realwidth ' + realwidth);
							pictureAngle = 1;
							break;
						case 1:
							_debug('---- 90 deg ----');
							document.getElementById('pictureBig').className = 'deg90';
							if ( empty(realheight) ) { realheight = document.getElementById('pictureBig').style.height.match(/\d*/); }
							if ( empty(realwidth) ) { realwidth = document.getElementById('pictureBig').style.width.match(/\d*/); }
							_debug('realheight ' + realheight + ', ' + 'realwidth ' + realwidth);
							if ( aspect == 1 ) {
				...
				case 117:
					...
					aspTimer = setTimeout(function () { aspectChange = 0; }, 3000);
					if ( aspectChange ) {
						if ( aspect < 4 ) {
							aspect++;
						} else {
				...
				case 116:
					switch ( pictureAngle ) {
						...
						case 1:
							...
							if ( empty(realheight) ) { realheight = document.getElementById('BigPict').style.height.match(/\d*/); }
							if ( empty(realwidth) ) { realwidth = document.getElementById('BigPict').style.width.match(/\d*/); }
							_debug('realheight ' + realheight);
							_debug('realwidth ' + realwidth);
							if ( aspect == 1 && win.width == 720 ) {
