...
MediaPlayer.onInit = function () {
	...
	this.$PlayerBufferBar = playerBufferBar;
	this.$PlayerProgressBar = playerProgressBar;
	this.$PlayerHeader = playerHeader;
	this.$PlayerFooter = playerFooter;
	this.$PlayerList = playerListBox;
	this.$SlideContainer = slideContainer;

	$playerHideplist.innerHTML = lang.playerBtnF2sh;
	playerHeaderSetting.innerHTML = lang.playerBtnMenu;
	playerHideplayer.innerHTML = lang.playerBtnInfo;
	playlistPrev.innerHTML = lang.playlistPrev;
	playlistNext.innerHTML = lang.playlistNext;
	// click for hidden info button
	this.handle.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.EventHandler({code: KEYS.INFO});
		return false;
	});
	// click for menu button
	playerHeaderSetting.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
	...
	// click for f2 button
	$playerHideplist.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.togglePlaylist();
		return false;
	});
	// click for info button
	playerHideplayer.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.EventHandler({code: KEYS.INFO});
		return false;
	});
	// click for exit button
	playerExit.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.exit();
		return false;
	});
	// click for pause button
	playerPause.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
	...
	// click for ff button
	playerREW.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
	...
	// click for fr button
	playerFFWD.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.setPos(1);
		return false;
	});
	// click for pg_up button
	playlistPrev.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.EventHandler({code: KEYS.PAGE_UP});
		return false;
	});
	// click for pg_dw button
	playlistNext.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		MediaPlayer.EventHandler({code: KEYS.PAGE_DOWN});
		return false;
	});
	...
	this.ModalMenu.handle.addEventListener('click', function ( e ) {
		e.preventDefault();
		e.stopPropagation();
		self.ModalMenu.Show(false);
		console.log('ModalMenu-clicked');
		return false;
	});
	...
	this.ModalMenu.Menu.g3d = this.ModalMenu.Menu.AddGroup('g3d', lang.mediaMenuGroups['g3d'], {
		onclick: function () {
			...
			if ( gSTB.Set3DConversionMode ) { gSTB.Set3DConversionMode(this.data); }
	...
	this.ModalMenu.Menu.gslideOn.ioff = this.ModalMenu.Menu.AddItem(this.ModalMenu.Menu.gslideOn, 1, lang.mediaMenuOff, {
		data  : 0,
		marked: true
	});
...
MediaPlayer.togglePlaylist = function () {
	if ( this.list.length > 1 ) {
		if ( this.playListShow && this.infoFlag ) {
			...
			$playerHideplist.innerHTML = lang.playerBtnF2hd;
			this.showInfo(true);
...
MediaPlayer.EventHandler = function ( e ) {
	...
	switch ( e.code ) {
		case KEYS.UP:
		case KEYS.DOWN:
			if ( this.playListShow && this.infoFlag ) { this.PlayList.EventHandler(e); }
			break;
		case KEYS.PAGE_DOWN:
		...
		case KEYS.LEFT:
		case KEYS.REWIND:
			if ( this.type === MEDIA_TYPE_VIDEO || this.type === MEDIA_TYPE_AUDIO ) { this.setPos(-1); }
			break;
		case KEYS.RIGHT:
		case KEYS.FORWARD:
			if ( this.type === MEDIA_TYPE_VIDEO || this.type === MEDIA_TYPE_AUDIO ) { this.setPos(1); }
...
MediaPlayer.preparePlayer = function ( list, parent, fullScreen, play, show ) {
	...
	this.$PlayerList.style.display = 'none';
	if ( this.list.length > 1 ) {
		$playerHideplist.style.display = 'block';
		$playerHideplist.innerHTML = lang.playerBtnF2sh;
	} else {
		$playerHideplist.style.display = 'none';
	}
...
MediaPlayer.play = function () {
	...
	if ( deviceProxy.length < 1 ) {
		gSTB.Play((this.obj.sol ? this.obj.sol + ' ' : 'auto ') + this.obj.url);
	} else {
		gSTB.Play((this.obj.sol ? this.obj.sol + ' ' : 'auto ') + this.obj.url, deviceProxy);
	}
};
...
MediaPlayer.setPos = function ( a ) {
	...
	to = 0;
	for ( i = 0; i < this.posIntervals.length; i++ ) { to += a * this.posIntervals[i]; }
	if ( to > 1800 ) { to = 1800; }
	if ( to < -1800 ) { to = -1800; }
	this.pos += to;
	...
	px = Math.round(this.pos / this.totalTime * this.progress[screen.height]);
...
MediaPlayer.event = function ( e, info ) {
	...
	switch ( e ) {
		case 1: // The player reached the end of the media content or detected a discontinuity of the stream.
			...
			if ( currCPage === MediaPlayer ) { MediaPlayer.exit(); } else { MediaPlayer.end(); }
			break;
		...
		case 4: // Video and/or audio playback has begun.
			...
			if ( !MediaPlayer.playNow ) { ListPage.subscribeEvents[MediaPlayer.EVENT_START].call(ListPage); }
			switch ( MediaPlayer.type ) {
				case MEDIA_TYPE_AUDIO:
				case MEDIA_TYPE_VIDEO:
					MediaPlayer.runner.start();
					MediaPlayer.timer.showInfo = window.setTimeout(function () {
						echo('showInfo_3');
						MediaPlayer.showInfo(false);
					}, 5000);
					break;
		...
		case 32: // connect HDMI device
			...
			if ( gSTB.GetStandByStatus() && environment.hdmi_event_delay ) {
				standBy.standByOffTimer = setTimeout(function () { standBy.toggleMode(); }, environment.hdmi_event_delay * 1000);
			}
			break;
		case 33: // disconnect HDMI device
			...
			clearTimeout(standBy.standByOffTimer);
			if ( !gSTB.GetStandByStatus() && environment.hdmi_event_delay ) {
				standBy.standByOnTimer = setTimeout(function () { standBy.toggleMode(); }, environment.hdmi_event_delay * 1000);
			}
			break;
		case 35:
		case 34:
...
MediaPlayer.aspect = function () {
	...
	$ico = this.handle.querySelector('.mediaHeaderIco');
	...
	this.timer.hideAspect = window.setTimeout(function () { $ico.style.display = 'none'; }, 2000);
};
...
MediaPlayer.end = function () {
	...
	// call subscriber hook
	ListPage.subscribeEvents[this.EVENT_STOP].call(ListPage);
	this.obj = null;
	this.runner.stop();
...
MediaPlayer.exit = function () {
	...
	if ( this.end() ) {
		...
		ListPage.subscribeEvents[this.EVENT_EXIT].call(ListPage);
		this.Show(false);
	}
...
MediaPlayer.showInfo = function ( show, hidetime, showHeader ) {
	...
	if ( show === false || show === true ) {
		if ( show === this.infoFlag ) { return false; }
		this.infoFlag = !show;
	}
	if ( this.infoFlag ) {
...
MediaPlayer.runner = {
	...
	start: function () {
		...
		px = Math.round(MediaPlayer.curTime / MediaPlayer.totalTime * MediaPlayer.progress[screen.height]);
...
MediaPlayer.setAudioMenu = function () {
	...
	if ( audArr.length > 0 && audArr[0] && audArr[0].pid ) {
		...
		for ( i = 0; i < audArr.length; i++ ) {
			...
			this.ModalMenu.Menu.AddItem(this.ModalMenu.Menu.gaudio, lang_info[1], lang_info[0], {
				data  : audArr[i].pid,
				marked: fl
			});
			if ( fl ) {
	...
	if ( subArr.length > 0 ) {
		this.ModalMenu.Menu.AddItem(this.ModalMenu.Menu.gsubtitle, 'OFF', lang.mediaMenuOff, {
			data  : 'OFF',
			marked: this.subtitles_on !== true
		});
		for ( i = 0; i < subArr.length; i++ ) {
			...
			this.ModalMenu.Menu.AddItem(this.ModalMenu.Menu.gsubtitle, lang_info[1], lang_info[0], {
				data  : subArr[i].pid,
				marked: currSub === subArr[i].pid && this.subtitles_on === true
			});
		}
	} else {
...
PlayList.prototype.Add = function ( name, attrs, states ) {
	...
	item = CScrollList.prototype.Add.call(this, [body, star], {
		star         : star,
		data         : {
			index  : attrs.index || attrs.index === 0 ? attrs.index : '',
			url    : attrs.url ? attrs.url : '',
			pos    : attrs.pos || attrs.pos === 0 ? attrs.pos : '',
			parentI: attrs.parentI || attrs.parentI === 0 ? attrs.parentI : ''
		},
