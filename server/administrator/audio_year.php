...
if (@$_GET['del'] && !empty($_GET['id'])){
    ...
    Mysql::getInstance()->delete('audio_years', array('id' => intval($_GET['id'])));
...
if (!empty($_POST)){
    ...
    if (empty($_POST['name'])){
        ...
        $year_id = Mysql::getInstance()->insert('audio_years', array(
            'name'     => $_POST['name'],
            'modified' => 'NOW()'
        ))->insert_id();
        ...
        $year_id = intval($_GET['id']);

        Mysql::getInstance()->update('audio_years', array(
            'name'     => $_POST['name'],
            'modified' => 'NOW()'
        ), array('id' => $year_id));
...
$items = Mysql::getInstance()->from('audio_years');
...
if (isset($_GET['id'])){
    $current_year = Mysql::getInstance()->from('audio_years')->where(array('id' => (int) $_GET['id']))->get()->first();
}
?>
<body>
...
<table align="center" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td align="center" valign="middle" width="100%" bgcolor="#88BBFF">
            <font size="5px" color="White"><b>&nbsp;<?= _('AUDIO YEARS')?>&nbsp;</b></font>
        </td>
    </tr>
    <tr>
        <td>
        ...
        <td>
            ...
            <center>
                ...
                <tr>
                    ...
                    <td>
                        <input name="name" type="text" value="<?= !empty($current_year) ? $current_year['name'] : '' ?>">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="button" value="<?= htmlspecialchars(_('New'), ENT_QUOTES)?>" onclick="document.location='audio_year.php'">
                    </td>
                </tr>
                </table>
                </form>
