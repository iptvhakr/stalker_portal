...
if (@$_GET['del']){
    $query = "delete from main_city_info where id=".intval(@$_GET['id']);
    $rs=$db->executeQuery($query);
    header("Location: city_info.php");
}

if (isset($_GET['status']) && @$_GET['id']){
    $query = "update main_city_info set status='".intval(@$_GET['status'])."' where id=".intval(@$_GET['id']);
    $rs=$db->executeQuery($query);
    header("Location: city_info.php");
}
...
if (!$error){
    ...
    if (@$_GET['save'] && !$error){
        ...
        if(@$_POST['title'] && @$_POST['number']){
            ...
            $query = "insert into main_city_info (
                                        num,
                                        title,
                                        number
                                        ) 
...
<title>
Редактирование списка экстренных служб
</title>
</head>
<body>
...
<tr>
    ...
    <font size="5px" color="White"><b>&nbsp;Редактирование списка экстренных служб&nbsp;</b></font>
    </td>
</tr>
<tr>
    <td width="100%" align="left" valign="bottom">
        <a href="index.php"><< Назад</a> | <a href="help_city_info.php">Справочные службы</a> | <a href="other_city_info.php">Другие службы</a>
    </td>
</tr>
<tr>
    <td align="center">
...
while(@$rs->next()){
    ...
    echo "<a href='#' onclick='if(confirm(\"Удалить данную запись?\")){document.location=\"city_info.php?del=1&id=".$arr['id']."\"}'>del</a>&nbsp;&nbsp;\n";
...
if (@$_GET['edit']){
    $query = "select * from main_city_info where id=".intval(@$_GET['id']);
    $rs=$db->executeQuery($query);
    while(@$rs->next()){
        $arr=$rs->getCurrentValuesAsHash();
        $title = $arr['title'];
...
<tr>
    ...
    <table align="center">
        ...
        <tr>
           ...
           <td>
            <input type="button" value="Сохранить" onclick="save()">&nbsp;<input type="button" value="Новый" onclick="document.location='city_info.php'">
           </td>
        </tr>
    </table>
    </form>
