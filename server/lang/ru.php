<?php

$words = array();

$words['all_abc'] = array('*','А','Б','В','Г','Д','Е','Ё','Ж','З','И','Й','К','Л','М','Н','О','П','Р','С','Т','У','Ф','Х','Ц','Ч','Ш','Щ','Ъ','Ы','Ь','Э','Ю','Я');

$words['all_months'] = array(
                    'январь',
                    'февраль',
                    'март',
                    'апрель',
                    'май',
                    'июнь',
                    'июль',
                    'август',
                    'сентябрь',
                    'октябрь',
                    'ноябрь',
                    'декабрь'
                );
                
$words['all_title'] = 'Все';

$words['no_ch_info'] = '[Расписание отсутствует]';

$words['vod_today'] = 'сегодня';
$words['vod_yesterday'] = 'вчера';
$words['vod_last_week'] = 'последние 7 дней';

$words['records_time_h'] = 'ч';
$words['records_time_m'] = 'м';
$words['records_time_s'] = 'с';

// weather
$words['weather_comfort'] = 'Комфорт';
$words['weather_pressure'] = 'Давление';
$words['weather_mmhg'] = 'мм. рт.ст.';
$words['weather_wind'] = 'Ветер';
$words['weather_speed'] = 'м/с';
$words['weather_humidity'] = 'Влажность';

// karaoke
$words['karaoke_view'] = 'ОТОБРАЖЕНИЕ';
$words['karaoke_sort'] = 'СОРТИРОВКА';
$words['karaoke_search'] = 'ПОИСК';
$words['karaoke_sampling'] = 'ВЫБОРКА';
$words['karaoke_by_letter'] = 'ПО БУКВЕ';
$words['karaoke_by_performer'] = 'по исполнителю';
$words['karaoke_by_title'] = 'по названию';
$words['karaoke_title'] = 'КАРАОКЕ';

// layer
$words['layer_page'] = 'СТР';
$words['layer_from'] = 'ИЗ';
$words['layer_found'] = 'НАЙДЕНО';
$words['layer_records'] = 'ЗАПИСЕЙ';
$words['layer_loading'] = 'ЗАГРУЗКА...';

// media browser
$words['mbrowser_title'] = 'USB Устройство';
$words['mbrowser_connected'] = 'подключено';
$words['mbrowser_disconnected'] = 'отключено';
$words['mbrowser_not_found'] = 'не найдено';

// player
$words['player_limit_notice'] = 'Количество подключений ограничено.<br>Попробуйте позже';
$words['player_file_missing'] = 'Файл отсутствует';
$words['player_server_error'] = 'Ошибка сервера';
$words['player_server_unavailable'] = 'Сервер недоступен';
$words['player_series'] = 'серия';
$words['player_series_uc'] = 'СЕРИЯ';
$words['player_track'] = 'Дорожка';
$words['player_off'] = 'Отключить';
$words['player_subtitle'] = 'Субтитры';
$words['player_claim'] = 'Пожаловаться';
$words['player_on_sound'] = 'на звук';
$words['player_on_video'] = 'на изображение';
$words['player_audio'] = 'Аудио';
$words['player_ty'] = 'Спасибо, Ваше мнение будет учтено';
$words['series_by_one_play'] = 'одну серию';
$words['series_continuously_play'] = 'непрерывно';

// radio
$words['radio_title'] = 'РАДИО';

// tv
$words['tv_view'] = 'ОТОБРАЖЕНИЕ';
$words['tv_sort'] = 'СОРТИРОВКА';
$words['tv_favorite'] = 'ИЗБРАННОЕ';
$words['tv_move'] = 'ДВИГАТЬ';
$words['tv_by_number'] = 'по номеру';
$words['tv_by_title'] = 'по имени';
$words['tv_only_favorite'] = 'только избранное';
$words['tv_list'] = 'список';
$words['tv_list_w_info'] = 'список с инфо';
$words['tv_title'] = 'ТВ';

// vclub info
$words['vclub_info'] = 'информация о фильме';
$words['vclub_year'] = 'Год';
$words['vclub_genre'] = 'Жанр';
$words['vclub_length'] = 'Время';
$words['vclub_minutes'] = 'мин';
$words['vclub_director'] = 'Режиссер';
$words['vclub_cast'] = 'В ролях';

// vclub
$words['vclub_view'] = 'ОТОБРАЖЕНИЕ';
$words['vclub_sort'] = 'СОРТИРОВКА';
$words['vclub_search'] = 'ПОИСК';
$words['vclub_fav'] = 'ИЗБРАННОЕ';
$words['vclub_sampling'] = 'ВЫБОРКА';
$words['vclub_other'] = 'ДРУГОЕ';
$words['vclub_find'] = 'НАЙТИ';
$words['vclub_by_letter'] = 'ПО БУКВЕ';
$words['vclub_by_genre'] = 'ПО ЖАНРУ';
$words['vclub_by_year'] = 'ПО ГОДУ';
$words['vclub_search_box'] = 'поиск';
$words['vclub_query_box'] = 'выборка';
$words['vclub_by_title'] = 'по имени';
$words['vclub_by_addtime'] = 'по добавлению';
$words['vclub_top'] = 'по популярности';
$words['vclub_only_hd'] = 'только HD';
$words['vclub_only_favorite'] = 'только избранное';
$words['vclub_not_ended'] = 'недосмотренное';
$words['vclub_list'] = 'список';
$words['vclub_list_w_info'] = 'список с инфо';
$words['vclub_title'] = 'ВИДЕОКЛУБ';

// common
$words['cut_off_msg'] = 'Ваша приставка отключена.<br/> Свяжитесь с провайдером.';
$words['month_arr'] = array('ЯНВАРЯ', 'ФЕВРАЛЯ', 'МАРТА', 'АПРЕЛЯ', 'МАЯ', 'ИЮНЯ', 'ИЮЛЯ', 'АВГУСТА', 'СЕНТЯБРЯ', 'ОКТЯБРЯ', 'НОЯБРЯ', 'ДЕКАБРЯ');
$words['week_arr'] = array('ВОСКРЕСЕНЬЕ', 'ПОНЕДЕЛЬНИК', 'ВТОРНИК', 'СРЕДА', 'ЧЕТВЕРГ', 'ПЯТНИЦА', 'СУББОТА');
$words['week_short_arr'] = array('Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб');
$words['year'] = 'г';

// pvr
$words['records_title'] = 'ЗАПИСИ';

// ears
$words['ears_back'] = 'Н<br>А<br>З<br>А<br>Д';
$words['ears_about_movie'] = 'О<br><br>Ф<br>И<br>Л<br>Ь<br>М<br>Е';
$words['ears_tv_guide'] = 'Т<br>В<br><br>Г<br>И<br>Д';

// settings
$words['settings_title'] = 'НАСТРОЙКИ';
$words['parent_settings_cancel'] = 'ОТМЕНА';
$words['parent_settings_save'] = 'СОХРАНИТЬ';
$words['parent_settings_old_pass'] = 'Текущий пароль';
$words['parent_settings_title'] = 'РОДИТЕЛЬСКИЕ НАСТРОЙКИ';
$words['parent_settings_title_short'] = 'РОДИТЕЛЬСКИЕ';
$words['parent_settings_new_pass'] = 'Новый пароль';
$words['parent_settings_repeat_new_pass'] = 'Повтор нового пароля';

$words['settings_saved'] = 'Настройки сохранены';
$words['settings_saved_reboot'] = 'Настройки сохранены.<br>Приставка будет перезагружена. Нажмите OK.';
$words['settings_check_error'] = 'Ошибка заполнения полей';

$words['localization_settings_title'] = 'ЛОКАЛИЗАЦИЯ';
$words['localization_label'] = 'Язык интерфейса';

$words['empty'] = 'ПУСТО';

$words['course_title'] = 'Курс НБУ на ';

$words['dayweather_title'] = 'ПОГОДА';
$words['dayweather_pressure'] = 'давл.:';
$words['dayweather_mmhg'] = 'мм рт.ст.';
$words['dayweather_wind'] = 'ветер:';
$words['dayweather_speed'] = 'м/с';

$words['infoportal_title'] = 'ИНФОПОРТАЛ';

$words['cityinfo_title'] = 'ГОРОДСКАЯ СПРАВКА';
$words['cityinfo_main'] = 'экстренные';
$words['cityinfo_help'] = 'справочные';
$words['cityinfo_other'] = 'разное';
$words['cityinfo_sort'] = 'СЛУЖБЫ';

$words['horoscope_title'] = 'ГОРОСКОП';

$words['anecdote_title'] = 'АНЕКДОТЫ';

$words['anecdote_goto'] = 'ПЕРЕХОД';
$words['anecdote_like'] = 'НРАВИТСЯ';
$words['anecdote_bookmark'] = 'ЗАКЛАДКА';
$words['anecdote_to_bookmark'] = 'К ЗАКЛАДКЕ';

$words['anecdote_pagebar_title'] = 'АНЕКДОТ';

$words['mastermind_title'] = 'ИГРА БЫКИ И КОРОВЫ';
$words['mastermind_rules'] = 'ПРАВИЛА ИГРЫ';
$words['mastermind_rating'] = 'РЕЙТИНГ';
$words['mastermind_bull'] = 'Б';
$words['mastermind_cow'] = 'К';
$words['mastermind_rules_text'] = 'Ваша задача отгадать число из четырех неповторяющихся цифр, причем первая из них - не ноль. Каждое Ваше предположение будет сравниваться с числом, которое загадала приставка. Если Вы отгадали цифру, но она стоит не на своем месте, то это КОРОВА (К). Если Вы отгадали и цифру, и её место, то это БЫК (Б).<br><br><table width="100%" cellpadding="3" cellspacing="3"><tr><td width="100" style="color:#ffffff;">&larr; &rarr;</td><td style="color:#3f81cc;">Перемещение курсора</td></tr><tr><td style="color:#ffffff;">0-9</td><td style="color:#3f81cc;">Ввод цифр в ячейки</td></tr><tr><td style="color:#ffffff;">ОК</td><td style="color:#3f81cc;">Подтверждение хода</td></tr><tr><td style="color:#ffffff;">Page +/-</td><td style="color:#3f81cc;">Перемещение по страницам истории ходов</td></tr></table>';

$words['msg_service_off'] = 'Услуга отключена';
$words['msg_channel_not_available'] = 'Канал недоступен';

$words['epg_title'] = 'ТВ Гид';
$words['epg_record'] = 'ЗАПИСАТЬ';
$words['epg_remind'] = 'НАПОМНИТЬ';
$words['epg_memo']   = 'Напоминание';
$words['epg_goto_ch']   = 'Перейти на канал';
$words['epg_close_msg']   = 'Закрыть окно';
$words['epg_on_ch']   = 'на канале';
$words['epg_now_begins']   = 'сейчас начинается';
$words['epg_on_time'] = 'в';
$words['epg_started'] = 'началось';

?>