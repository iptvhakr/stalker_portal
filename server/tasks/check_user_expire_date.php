...
if (Config::getSafe('enable_internal_billing', false)){
    $ids = Mysql::getInstance()->from("`users`")
        ->where(array(
            "(TO_DAYS(`expire_billing_date`) - TO_DAYS(NOW())) < 0 AND CAST(`expire_billing_date` AS CHAR) <> '0000-00-00 00:00:00' AND 1=" => 1,
            'status'=>0))
        ->get()->all('id');
    Mysql::getInstance()->update("`users`", array('status'=>1), array(" `id` IN ('" . implode("', '", $ids) . "') AND 1=" => 1, 'status'=>0));
