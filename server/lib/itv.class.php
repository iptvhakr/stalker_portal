...
class Itv extends AjaxResponse implements \Stalker\Lib\StbApi\Itv {
    ...
    public function getRealChannelByChannelId($ch_id, $link_id = null) {
        ...
        if (!empty($channel['streamer_id'])) {
            ...
            $cache->set($this->stb->id . '_playback', array(
                    'type' => 'tv-channel',
                    'id' => $channel['id'],
    ...
    public function setPlayed() {
        ...
        $this->db->update('users', array('time_last_play_tv' => 'NOW()'), array('id' => $this->stb->id));
    ...
    public function saveFav(array $fav_array, $uid) {
        ...
        if ($this->fav_itv === null) {
            $this->fav_itv = $fav_itv_arr = $this->db->from('fav_itv')->where(array('uid' => intval($uid)))->use_caching(array('fav_itv.uid=' . intval($uid)))->get()->first();
        } else {
            $fav_itv_arr = $this->fav_itv;
        }

        if (empty($fav_itv_arr)) {
            return $this->db->use_caching(array('fav_itv.uid=' . intval($uid)))->insert('fav_itv', array(
                        'uid' => (int)$uid,
                        'fav_ch' => $fav_ch_str,
                        'addtime' => 'NOW()'
                    ))->insert_id();
        } else {
            return $this->db->use_caching(array('fav_itv.uid=' . intval($uid)))->update('fav_itv', array(
                        'fav_ch' => $fav_ch_str,
                        'addtime' => 'NOW()'
    ...
    public function getFav($uid = null) {
        ...
        if ($this->fav_itv === null) {
            $this->fav_itv = $fav_itv_ids_arr = $this->db->from('fav_itv')->where(array('uid' => intval($uid)))->use_caching(array('fav_itv.uid=' . intval($uid)))->get()->first();
        } else {
    ...
    public function getListByNumber() {
        ...
        $this->db->from('itv')->where(array('status' => 1));
    }
    ...
    public function getFavIds() {
        ...
        $fav_ids = $this->db->from('itv')->in('id', $fav)->where(array('status' => 1))->orderby('field(id,' . $fav_str . ')')->get()->all('id');
    ...
    public function getGenres() {
        ...
        if (in_array('dvb', Stb::getAvailableModulesByUid($this->stb->id)) && in_array(Stb::getInstance()->getParam('stb_type'), array(
                'MAG270',
                'MAG275',
                'AuraHD3'
            ))
        ) {
            array_unshift($genres, array(
                'id' => 'dvb',
                'title' => _('DVB')
            ));
        }

        if (Config::getSafe('show_pvr_filter_in_genres_list', false) && in_array(Stb::getInstance()->getParam('stb_type'), explode(',', Config::get('allowed_stb_types_for_local_recording')))
        ) {
            array_unshift($genres, array(
                'id' => 'pvr',
                'title' => _('Channels with PVR')
            ));
        }
    ...
    private function getData() {
        ...
        $this->db->from('itv')->where($where)->limit(self::max_page_items, $offset);

        if (Config::getSafe('enable_numbering_in_order', false) && Config::getSafe('order_itv_channel_as_adding', false) && (empty($_REQUEST['sortby']) || (!empty($_REQUEST['sortby']) && $_REQUEST['sortby'] == 'number'))) {
            $this->db->select(array('itv.*'))->join('service_in_package', 'itv.id', 'service_in_package.service_id', 'LEFT')->orderby('service_in_package.id', 'DESC')->groupby('service_in_package.service_id');
        }
    ...
    public static function getChannelNameById($id) {
        ...
        return Mysql::getInstance()->from('itv')->where(array('id' => $id))->use_caching(array('itv.id=' . intval($id)))->get()->first('name');
    }
    ...
    public static function getLinkById($id) {
        ...
        if (isset(self::$links_cache[strval($id)])) {
            ...
            $link = self::$links_cache[strval($id)] = Mysql::getInstance()->from('ch_links')->where(array('id' => $id))->use_caching(array('ch_links.id=' . intval($id)))->get()->first();
        }
    ...
    public static function getById($id) {
        if (isset(self::$channels_cache[strval($id)])) {
            ...
            $channel = self::$channels_cache[strval($id)] = Mysql::getInstance()->from('itv')->where(array('id' => intval($id)))->use_caching(array('itv.id=' . intval($id)))->get()->first();
        }
    ...
    private function getCensoredList() {
        ...
        if ($this->censored_channels === null) {
            $this->censored_channels = $list = Mysql::getInstance()->from('censored_channels')->where(array('uid' => intval($this->stb->id)))->use_caching(array('censored_channels.uid=' . intval($this->stb->id)))->get()->first();
        } else {
            $list = $this->censored_channels;
        }
    ...
    private function setCensoredList($list) {
        ...
        if ($this->censored_channels === null) {
            $this->censored_channels = $item = Mysql::getInstance()->from('censored_channels')->where(array('uid' => intval($this->stb->id)))->use_caching(array('censored_channels.uid=' . intval($this->stb->id)))->get()->first();
        } else {
            $item = $this->censored_channels;
        }
    ...
    public function getUrlsMapForChannels($ch_ids) {
        ...
        $channel_links = Mysql::getInstance()->from('ch_links')->where(array('status' => 1))->in('ch_id', $ch_ids)->orderby('priority, rand()')->get()->all();
    ...
    public static function getUrlsForChannel($ch_id, $for_pvr = false) {
        ...
        $channel_links = Mysql::getInstance()->from('ch_links')->where(array(
                'ch_id' => $ch_id,
                'status' => 1
            ))->orderby('priority, rand()')->get()->all();
...
var_dump($user_agent);
    ...
    public function saveDvbChannels() {
        ...
        if (empty($dvb_channels)) {
            return Mysql::getInstance()->insert('dvb_channels', array(
                    'uid' => $this->stb->id,
                    'channels' => json_encode($channels),
                    'modified' => 'NOW()'
                ))->insert_id();
        } else {
            return Mysql::getInstance()->update('dvb_channels', array(
                    'channels' => json_encode($channels),
                    'modified' => 'NOW()'
                ), array('uid' => $this->stb->id))->result();
        }
    }
    ...
    public function getDvbChannels() {
        ...
        $this->dvb_channels = array_map(function ($channel) use (&$ch_number) {
            ...
            $channel['id'] = (int)str_replace(array(
                'T',
                'C',
                '_'
            ), '', $channel['id']);
    ...
    public static function setChannelLinkStatus($link_id, $status) {
        ...
        if (strpos($link_id, 's') === 0) { // stream balanser monitoring
            ...
            if ($status == 0) {
                ...
                $other_good_balanser_links = Mysql::getInstance()->from('ch_link_on_streamer')->where(array(
                        'link_id' => $link['id'],
                        'id!=' => $balanser_link_id,
                        'monitoring_status' => 1
                    ))->get()->all();
        ...
        $good_links = Mysql::getInstance()->from('ch_links')->where(array(
            'ch_id' => $ch_id,
            'status' => 1
        ))->get()->all();
    ...
    public function getLinksForMonitoring($status = FALSE) {
        ...
        $result = Mysql::getInstance()->select('ch_links.*, itv.name as ch_name')->from('ch_links')->join('itv', 'itv.id', 'ch_links.ch_id', 'INNER')->where(array(
        ...
        $result = Mysql::getInstance()->select('ch_links.*, streamer_id, ch_link_on_streamer.id as streamer_link_id, itv.name as ch_name')->from('ch_links')->join('ch_link_on_streamer', 'link_id', 'ch_links.id', 'INNER')->join('itv', 'itv.id', 'ch_links.ch_id', 'INNER')->where(array(
                'ch_links.enable_monitoring' => 1,
...
// egdecast code
        $params = "ec_expire=" . (time() + Config::get($token_type)) . "&ec_clientip=" . Stb::getInstance()->ip;
        $td = mcrypt_module_open(MCRYPT_BLOWFISH, '', 'ncfb', '');
        $iv = "\0\0\0\0\0\0\0\0";
        $text = sprintf('ec_secure=%03d&%s', strlen($params) + 14, $params);
        $hash = FALSE;
        if (mcrypt_generic_init($td, Config::get('EDGECAST_KEY'), $iv) != -1) {
