...
class TvArchive extends Master implements \Stalker\Lib\StbApi\TvArchive
{
    ...
    private function createTemporaryToken($val){
        ...
        $result = $cache->set($key, $val, 0, 28800); // 28800 -  8 hours
    ...
    protected function getAllActiveStorages(){
        ...
        $data = $this->db->from('storages')->where(array('status' => 1, 'for_records' => 1))->where(array(' stream_server_type' => NULL, 'stream_server_type' => ''), 'OR ')->get()->all();
