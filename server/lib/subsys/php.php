...
class Subsys_JsHttpRequest_Php
{
    ...
    function setEncoding($enc)
    {
        ...
        // Manually parse QUERY_STRING because of damned Unicode's %uXX XX.
    ...
    function php2js($a)
    {
        ...
        $result = array();
        if ($isList) {
            foreach ($a as $v) $result[] = Subsys_JsHttpRequest_Php::php2js($v);
            return '[ ' . join(',', $result) . ' ]';
        } else {
            foreach ($a as $k=>$v) $result[] = Subsys_JsHttpRequest_Php::php2js($k) . ': ' . Subsys_JsHttpRequest_Php::php2js($v);
            return '{ ' . join(',', $result) . ' }';
        }
    }
    ...
    function _obHandler($text)
    {
        ...
        //header("Content-type: " . ($this->LOADER=="SCRIPT"? "application/javascript" : "text/plain") . "; charset=" . $this->SCRIPT_ENCODING);
...
//      $f = fopen("debug", "w"); fwrite($f, $text); fclose($f);
    ...
    /**
     * Decode all %uXX XX entities in string or array (recurrent).
    ...
    /**
     * Decode one %uXX XX entity (RE callback).
     */
