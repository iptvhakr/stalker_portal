...
/////////////////////////////////////////////////////////////////
//                                                             //
// getid3.avr.php - part of getID3()                           //
// See getid3.readme.txt for more details                      //
//                                                             //
/////////////////////////////////////////////////////////////////

function getAVRHeaderFilepointer(&$fd, &$ThisFileInfo) {
	...
	if ($ThisFileInfo['avr']['raw']['magic'] != '2BIT') {
		$ThisFileInfo['error'] .= "\n".'Expecting "2BIT" at offset '.$ThisFileInfo['avdataoffset'].', found "'.$ThisFileInfo['avr']['raw']['magic'].'"';
	...
	$ThisFileInfo['avr']['raw']['mono']        = BigEndian2Int(substr($AVRheader, 12,  2));
	$ThisFileInfo['avr']['bits_per_sample']    = BigEndian2Int(substr($AVRheader, 14,  2));
	$ThisFileInfo['avr']['raw']['signed']      = BigEndian2Int(substr($AVRheader, 16,  2));
	$ThisFileInfo['avr']['raw']['loop']        = BigEndian2Int(substr($AVRheader, 18,  2));
	$ThisFileInfo['avr']['raw']['midi']        = BigEndian2Int(substr($AVRheader, 20,  2));
	$ThisFileInfo['avr']['raw']['replay_freq'] = BigEndian2Int(substr($AVRheader, 22,  1));
	$ThisFileInfo['avr']['sample_rate']        = BigEndian2Int(substr($AVRheader, 23,  3));
	$ThisFileInfo['avr']['sample_length']      = BigEndian2Int(substr($AVRheader, 26,  4));
	$ThisFileInfo['avr']['loop_start']         = BigEndian2Int(substr($AVRheader, 30,  4));
	$ThisFileInfo['avr']['loop_end']           = BigEndian2Int(substr($AVRheader, 34,  4));
	$ThisFileInfo['avr']['midi_split']         = BigEndian2Int(substr($AVRheader, 38,  2));
	$ThisFileInfo['avr']['sample_compression'] = BigEndian2Int(substr($AVRheader, 40,  2));
	$ThisFileInfo['avr']['reserved']           = BigEndian2Int(substr($AVRheader, 42,  2));
	...
	if (($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']) != ($ThisFileInfo['avr']['sample_length'] * (($ThisFileInfo['avr']['bits_per_sample'] == 8) ? 1 : 2))) {
		$ThisFileInfo['warning'] .= "\n".'Probable truncated file: expecting '.($ThisFileInfo['avr']['sample_length'] * (($ThisFileInfo['avr']['bits_per_sample'] == 8) ? 1 : 2)).' bytes of audio data, found '.($ThisFileInfo['avdataend'] - $ThisFileInfo['avdataoffset']);
	}
