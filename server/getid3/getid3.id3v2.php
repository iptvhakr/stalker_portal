...
function ID3v2FrameProcessing($frame_name, $frame_flags, &$ThisFileInfo) {
	...
	if (isset($ThisFileInfo['id3v2']["$frame_name"]['datalength']) && ($ThisFileInfo['id3v2']["$frame_name"]['datalength'] == 0)) {
	...
	} elseif ((($ThisFileInfo['id3v2']['majorversion'] >= 3) && ($frame_name == 'TXXX')) || // 4.2.2 TXX X User defined text information frame
			(($ThisFileInfo['id3v2']['majorversion'] == 2) && ($frame_name == 'TXX'))) {     // 4.2.2 TXX  User defined text information frame
		//   There may be more than one 'TX XX' frame in each tag,
		//   but only one with the same description.
		// <Header for 'User defined text information frame', ID: 'TX XX'>
		// Text encoding     $xx
		...
		// excluding 'TXX X' described in 4.2.6.>
		// Text encoding                $xx
	...
	} elseif ((($ThisFileInfo['id3v2']['majorversion'] >= 3) && ($frame_name == 'WXXX')) || // 4.3.2 WXX X User defined URL link frame
			(($ThisFileInfo['id3v2']['majorversion'] == 2) && ($frame_name == 'WXX'))) {     // 4.3.2 WXX  User defined URL link frame
		//   There may be more than one 'WX XX' frame in each tag,
		//   but only one with the same description
		// <Header for 'User defined URL link frame', ID: 'WX XX'>
		// Text encoding     $xx
		// Description       <text string according to encoding> $00 (00)
		// URL               <text string>
		...
		// <Header for 'URL link frame', ID: 'W000' - 'WZZZ', excluding 'WXX'
		// described in 4.3.2.>
		// URL              <text string>
...
function LanguageLookup($languagecode, $casesensitive=false) {
	...
	// language is not known the string "XX X" should be used.
	switch ($languagecode) {
		case 'XXX':
		case 'xxx':
			return 'unknown';
