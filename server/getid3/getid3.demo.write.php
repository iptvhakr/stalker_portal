...
/////////////////////////////////////////////////////////////////
//                                                             //
// getid3.demo.write.php - part of getID3()                    //
...
if (isset($_POST['WriteOggCommentTagNow'])) {
...
} elseif (isset($_POST['WriteNewTagNow'])) {
	...
	$data['id3v2']['TXXX'][0]['data']        = 'getID3() v'.GETID3VERSION.' (www.getid3.org)';


	if ($_POST['WriteOrDelete'] == 'W') { // write tags

		if (!empty($_POST['VersionToEdit1'])) {
		...
		if (!empty($_POST['VersionToEditAPE'])) {
			echo 'APE tag'.(DeleteAPEtag($EditorFilename) ? '' : ' NOT').' successfully deleted<HR>';
		}

	}
}

echo '<A HREF="getid3.demo.check.php?listdirectory='.rawurlencode(realpath(dirname($EditorFilename))).'">Browse directory</A><BR>';
...
if ($EditorFilename) {
	...
	if (file_exists($EditorFilename)) {
		...
		if ($OldThisfileInfo['fileformat'] == 'mp3') {
			...
			echo '<INPUT TYPE="HIDDEN" NAME="WriteNewTagNow" VALUE="1">';
			...
			echo '<TR><TD ALIGN="CENTER" COLSPAN="2"><INPUT TYPE="CHECKBOX" NAME="VersionToEdit1" VALUE="1"> ID3v1 <INPUT TYPE="CHECKBOX" NAME="VersionToEdit2" VALUE="2" CHECKED> ID3v2 <INPUT TYPE="CHECKBOX" NAME="VersionToEditAPE" VALUE="APE" CHECKED> APE</TD></TR>';
