...
function getLAHeaderFilepointer(&$fd, &$ThisFileInfo) {
	...
	switch (substr($rawdata, $offset, 4)) {
		...
		case 'LA04':
			...
			$LaHeaderFlags                              = LittleEndian2Int(substr($rawdata, $offset, 1));
			$offset += 1;
			$ThisFileInfo['la']['seekable']             = (bool) ($LaHeaderFlags & 0x01);
			if ($ThisFileInfo['la']['version'] >= 0.4) {
				$ThisFileInfo['la']['high_compression'] = (bool) ($LaHeaderFlags & 0x02);
			}
