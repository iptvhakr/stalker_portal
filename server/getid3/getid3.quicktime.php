	...
	switch ($atomname) {
		...
		case 'stts': // Sample Table Time-to-Sample atom
			...
			for ($i = 0; $i < $atomstructure['number_entries']; $i++) {
				...
				safe_inc($FrameRateCalculatorArray[($ThisFileInfo['quicktime']['time_scale'] / $atomstructure['time_to_sample_table'][$i]['sample_duration'])], $atomstructure['time_to_sample_table'][$i]['sample_count']);
			}
			$sttsFramesTotal  = 0;
			$sttsSecondsTotal = 0;
			foreach ($FrameRateCalculatorArray as $frames_per_second => $frame_count) {
				if (($frames_per_second > 60) || ($frames_per_second == 0)) {
